...
function isEnglishTranslationLang(lang) {
  const s = String(lang || '').trim().toLowerCase();
  return s === '영어' || s === 'english' || s === '1' || s.includes('english') || s.includes('영어');
}

function pickEnglishTranslation(translations) {
  for (const t of asArray(translations)) {
    if (!isEnglishTranslationLang(t.trans_lang)) continue;
    return {
      word: t.trans_word || '',
...
function mergeSearchEntry(best, detailed, query) {
  ...
  // View enrichment can return empty translation fields — never wipe search glosses.
  return {
    ...best,
    ...detailed,
    word: best?.word || detailed?.word || trimmed,
    entryId: best?.entryId || detailed?.entryId || '',
    englishWord: preferNonEmpty(detailed?.englishWord, best?.englishWord),
    definition: preferNonEmpty(detailed?.definition, best?.definition),
    rawDefinitionKo: preferNonEmpty(detailed?.rawDefinitionKo, best?.rawDefinitionKo),
    pronunciation: preferNonEmpty(detailed?.pronunciation, best?.pronunciation),
    partOfSpeech: preferNonEmpty(detailed?.partOfSpeech, best?.partOfSpeech),
    partOfSpeechEn: preferNonEmpty(detailed?.partOfSpeechEn, best?.partOfSpeechEn),
    example: preferNonEmpty(detailed?.example, best?.example),
    sourceUrl: preferNonEmpty(detailed?.sourceUrl, best?.sourceUrl),
