...
async function searchWord(apiKey, query, options = {}) {
  ...
  if (!best) return { found: false, query: trimmed, total: 0, entry: null, candidates: [] };
    ...
    try {
      ...
      if (detailed) {
        return {
          ...
          query: trimmed,
          total,
          entry: { ...best, ...detailed, lastFetchedAt: new Date().toISOString() },
          candidates: items,
        };
      }
    } catch {
...
/**
 * Validate whether a word exists in the official dictionary.
 */
async function validateWord(apiKey, word, options = {}) {
  const result = await searchWord(apiKey, word, options);
  return {
    valid: result.found,
    word,
    entry: result.entry,
