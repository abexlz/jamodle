...
/**
 * Plain-text separator between Hangul syllables.
 * Commas create a spoken pause; set to half of the previous doubled gap (", , ").
...
function buildSyllableSsml(text) {
  ...
  return syllables.join(SYLLABLE_SEPARATOR);
}
...
async function synthesizeWithEdgeVoice(text, voiceName, prosody) {
  ...
  try {
    ...
    const spoken = buildSyllableSsml(text);
