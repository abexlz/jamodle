...
(function (global) {
  ...
  const STYLE_HREF = 'css/menu-hud.css?v=20260824c';
  const CHROME_HREF = 'css/app-chrome.css?v=20260823b';
  const PROFILE_STYLE_HREF = 'css/profile.css?v=20260823j';
  ...
  const DEPS = [
    ...
    ['DailyCalendarModal', 'js/daily-calendar-modal.js?v=20260823g'],
      ...
      <div class="menu-top-bar menu-hud-bar" id="${HUD_ID}">
        <div class="menu-top-bar-left menu-hud-left">
          <a class="menu-hud-spin menu-wheel-nav" href="chest-room.html" id="menu-wheel-nav" data-i18n-aria="chestRoom.pageNav">
            ${CHEST_ICON}
            <span class="menu-hud-spin-label" data-i18n="chestRoom.pageNav">Chests</span>
          </a>
  ...
  function inject() {
    ...
    if (bar) {
      ...
      if (spin && (!hasFancyImg || chestArt?.querySelector('svg'))) {
        ...
        const label = spin.querySelector('.menu-hud-spin-label');
        if (label) {
          label.setAttribute('data-i18n', 'chestRoom.pageNav');
          if (!label.textContent.trim()) label.textContent = 'Chests';
        }
      }
    } else {
